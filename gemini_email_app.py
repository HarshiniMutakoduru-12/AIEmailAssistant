...
from model_utils import init_model, process_message
...
def main():
    ...
    <div class="app-header">
        <div class="logo-container">
            <div class="logo-icon"><span style="color: white; font-size: 18px;">✨</span></div>
            <div class="logo-text">Gemini</div>
        </div>
    </div>
    """, unsafe_allow_html=True)
    ...
    if not messages:
        ...
        for idx, msg in enumerate(messages):
            if msg["role"] == "user":
                ...
                <div class="message-group">
                    <div class="ai-message-container">
                        <div class="ai-avatar"><span class="ai-avatar-icon">✨</span></div>
                        <div class="ai-content"><div class="ai-message">{msg["content"]}</div></div>
                    </div>
                </div>
                """, unsafe_allow_html=True)
    ...
    with col1:
        user_input = st.text_area(
            "", placeholder="Ask anything...",
            height=50, key=f"input_{st.session_state.input_key}",
            label_visibility="collapsed"
        )
    ...
    with col2:
        if st.button("➤", use_container_width=True):
            if user_input.strip():
                messages.append({"role": "user", "content": user_input, "id": str(uuid.uuid4())})
                with st.spinner(""):
                    response = process_message(user_input, st.session_state.model)
                messages.append({"role": "assistant", "content": response, "id": str(uuid.uuid4())})
                st.session_state.input_key += 1
                st.rerun()
